...
off <- read.table("../../../Dataset/offsets.txt", header=T, sep=",")
...
dat = list(Nin=ins, Nout=outs, X=X, off = off, n=n, p=p, H=H, m = m, I = I, mu.0 = rep(0,p))
inits = function() {list(theta.0=rep(0,p), theta.1=rep(0,p), theta.2=rep(0,p),
                         alpha.0 = matrix(rep(0, 2*p), nrow = p, ncol=2), alpha.1 = matrix(rep(0, 2*p), nrow = p, ncol=2),
                         alpha.2 = matrix(rep(0, 2*p), nrow = p, ncol=2), beta.0 = matrix(rep(0, H*p), nrow=p, ncol=H),
...
variable.names <- c("theta.0","theta.1","theta.2", "alpha.0","alpha.1","alpha.2","beta.0","beta.1","beta.2", "X0_pred", "X1_pred", "X2_pred")
