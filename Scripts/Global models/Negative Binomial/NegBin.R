...
###################################################################
## Define the data 
dat = list(Y=volume$Y, X=cbind(1,volume[,2:8]), n=35, mu.beta=rep(0,8), tau.beta=diag(rep(0.001,8)))

## A list of initial value for the MCMC algorithm in JAGS
inits = function() {list(beta=rep(0,8))}
...
## Save the model
save(outputRegress,file= 'NegBin_output.Rdata')
...
# the most seem to be irrelevant -> rain today, weekend the good ones
...
sz = dim(data.out)[1]
x11()
plot(1:35, pred_mean, pch = 16, ylim=c(0,30000), col = 'red', type ='o', xlab = 'day', ylab='traffic', main ='comparison between actual and predicted Y')
