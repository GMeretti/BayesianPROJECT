...
abline(v=42.5, col ='darkred')

for(ii in 1:7)
{
  points(ii+42, quantile(future_y[,ii], 0.05), col = 'darkred', pch = 16)
  points(ii+42, quantile(future_y[,ii], 0.95), col = 'darkred', pch = 16)
  segments(ii+42, quantile(future_y[,ii] , 0.05), ii+42, quantile(future_y[,ii], 0.95), col  = 'darkred')
  if(ii!=7)
  {
    lines(c(ii+42, ii+43), c(quantile(future_y[,ii] , 0.05), quantile(future_y[,ii+1], 0.05)), col = 'darkred', lty = 3)
    lines(c(ii+42, ii+43), c(quantile(future_y[,ii] , 0.95), quantile(future_y[,ii+1], 0.95)), col = 'darkred', lty = 3)
  }
}
