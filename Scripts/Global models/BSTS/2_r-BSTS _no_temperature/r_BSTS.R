#################################################
#####  residual no temperature BSTS models  #####
#################################################
## Prerequisites
rm(list=ls())
library(rjags)
...
## Define the data
lag  =5
dat = list(Y=volume$Y,  Rt=w, Wd = rep(c(rep(0,5),rep(1,2)),6), n=42, lag=lag,
...
x11()
par(mfrow=c(1,dim(alpha.data)[2]))
for(i in 1:dim(alpha.data)[2]){
  acf(alpha.data[,i],lwd=3,col="red3", main=expression(alpha))
}
...
for(ii in 1:42)
{
  points(ii, quantile(pred_y[1:n,ii], 0.05), col = 'darkred', pch = 16)
  points(ii, quantile(pred_y[1:n,ii], 0.95), col = 'darkred', pch = 16)
  segments(ii, quantile(pred_y[1:n,ii] , 0.05), ii, quantile(pred_y[1:n,ii], 0.95), col  = 'darkred')
  if(ii!=42)
  {
    lines(c(ii, ii+1), c(quantile(pred_y[1:n,ii] , 0.05), quantile(pred_y[1:n,ii+1], 0.05)), col = 'darkred', lty = 3)
    lines(c(ii, ii+1), c(quantile(pred_y[1:n,ii] , 0.95), quantile(pred_y[1:n,ii+1], 0.95)), col = 'darkred', lty = 3)
  }
}
...
for(ii in 1:n)
{
  for(jj in 1:7)
  {
    ...
    future_rho[ii,jj+1] = future_rho[ii,jj]*alpha.data[ii,1] + rnorm(n=1, mean= 0, sd = 1/sqrt(tau.data[ii,5]))
    future_gamma[ii,jj+6] = -sum(future_gamma[ii,jj:(jj+5)]) + rnorm(n=1, mean=0, sd = 1/sqrt(tau.data[ii,4]))
    future_y[ii,jj] = (beta.data[jj,1]+nu.data[jj,1]*((ii%%7)==0||(ii%%7)==6))* datas[jj,1] + future_mu[ii,jj+1] + future_rho[ii,jj+1]+future_gamma[ii,jj+6]+ rnorm(n=1, mean=0, sd = 1/sqrt(tau.data[ii,1]))
  }
}
...
sum(outlier) # number of obs falling outside the credible intervals: 1 out of 42
