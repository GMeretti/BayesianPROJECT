...
## Compute model
modelRegress=jags.model("BSTS.bug",data=dat,inits=inits,
                        n.adapt=10000,n.chains=2)

## Save the model
save(modelRegress, file='BSTS_model.Rdata') 
...
## Save output
save(outputRegress,file='BSTS_output.Rdata')
##-------------------------------------------##
...
pred = as.vector(expected.mu+expected.gamma + (expected.beta[1]+expected.nu[1]*rep(c(rep(0,5),rep(1,2)),6))*w +(expected.beta[2]+expected.nu[2]*rep(c(rep(0,5),rep(1,2)),6))*volume$T) 
...
for(ii in 1:n)
{
  for(jj in 1:7)
  {
    ...
    future_y[ii,jj] = (beta.data[jj,1]+nu.data[jj,1]*((ii%%7)==0||(ii%%7)==6))* datas[jj,1] + (beta.data[jj,2]+nu.data[jj,2]*((ii%%7)==0||(ii%%7)==6))*datas[jj,2] + future_mu[ii,jj+1] + future_gamma[ii,jj+6]+ rnorm(n=1, mean=0, sd = 1/sqrt(tau.data[ii,1]))
  }
}
...
## Representaton
## Representation of the posterior chain 
chain <- beta.data[,1]
...
##### PREDICTION OF ONE WEEK  #######à
dat = list(Y=volume$Y[1:35],  Rt=w, Wd = rep(c(rep(0,5),rep(1,2)),5), 
           T=volume$T[1:35], n=35, lag=lag,
           mean_mu=mean_mu, mean_gamma=mean_gamma)
...
gamma.data = as.data.frame(data.out[,grepl("gamma", names(data.out))])
gamma.data = gamma.data[,(1+lag):(dim(gamma.data)[2]-1)]
pred_y <- as.data.frame(data.out[,grepl("Ypred", names(data.out))]) # samples of the posterior for t = 1: 35
...
datas = cbind(w[36:42], volume$T[36:42])
for(ii in 1:n)
{
  for(jj in 1:7)
  {
    ...
    future_mu[ii, jj+1] = future_delta[ii,jj] + future_mu[ii,jj] + rnorm(n=1, mean=0, sd = 1/sqrt(tau.data[ii, 2]))
    future_gamma[ii,jj+6] = -sum(future_gamma[ii,jj:(jj+5)]) + rnorm(n=1, mean=0, sd = 1/sqrt(tau.data[ii,4]))
    future_y[ii,jj] = max((beta.data[jj,1]+nu.data[jj,1]*((ii%%7)==0||(ii%%7)==6))* datas[jj,1] + (beta.data[jj,2]+nu.data[jj,2]*((ii%%7)==0||(ii%%7)==6))*datas[jj,2] + future_mu[ii,jj+1] + future_gamma[ii,jj+6]+ rnorm(n=1, mean=0, sd = 1/sqrt(tau.data[ii,1])),0)
  }
}

x11()
plot(1:42, c(colMeans(pred_y),colMeans(future_y)), pch = 16, col = 'red', type ='o', xlab = 'day', ylab='traffic', main ='comparison between actual and predicted Y', ylim=c(0,15000))
points(1:42, volume$Y, pch=15)
for(ii in 1:35)
{
  points(ii, quantile(pred_y[,ii], 0.05), col = 'darkred', pch = 16)
...
abline(v=35.5, col='darkred', lty=2, lwd=3)

for(ii in 1:7)
{
  points(ii+35, quantile(future_y[,ii], 0.05), col = 'darkred', pch = 16)
  points(ii+35, quantile(future_y[,ii], 0.95), col = 'darkred', pch = 16)
  segments(ii+35, quantile(future_y[,ii] , 0.05), ii+35, quantile(future_y[,ii], 0.95), col  = 'darkred')
  if(ii!=7)
  {
    lines(c(ii+35, ii+36), c(quantile(future_y[,ii] , 0.05), quantile(future_y[,ii+1], 0.05)), col = 'darkred', lty = 3)
    lines(c(ii+35, ii+36), c(quantile(future_y[,ii] , 0.95), quantile(future_y[,ii+1], 0.95)), col = 'darkred', lty = 3)
  }
}
